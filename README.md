...
|---|---|---|---|
| [Round 4](benchmarks/rounds/round4-hardware-lottery/) | Sep 24-25, 2026 | Rent two pods of the identical advertised GPU spec at the same time, run the identical job list on both: does "the same spec" mean the same real performance, and does an identical seed reproduce the same output across different physical GPUs | Confirmed different physical units (GPU UUID), and a real configuration difference (one pod had a 500 W power limit vs. the usual 400 W) -- but speed differed by only 1.6% on average. Reproducibility: small pixel-level drift (0.8-1.3) across different hardware, far below the threshold for genuinely different content (~50), and visually indistinguishable. |
| [Round 3](benchmarks/rounds/round3-concurrent-batching/) | Sep 23-24, 2026 | Does batching 2 or 4 different prompts into one GPU call change cost-per-video versus one request at a time (round 1's exact prompts and seeds, reused for direct comparison) | Batching helps, modestly: 4% cheaper at batch=2, 6% cheaper at batch=4, vs. one at a time. A units bug made the live automated gate report 0 of 7 clips delivered; the actual files were verified by hand, an offline recheck confirmed all 7 were genuinely correct, and the bug was fixed. |
| [Round 2](benchmarks/rounds/round2-capability-mismatch/) | Sep 22-23, 2026 | Prompts asking for audio, 4K/8K, wrong aspect ratio, camera/lens jargon, or a longer duration than this pipeline can produce (5 of 6 planned clips; one skipped by design, see round page) | Cost matched round 1 within 1% on a different pod ($0.0527/video-s, 1.32x fal) -- a second, independent confirmation of that figure. 0 of 6 clips had any audio track, including the one whose prompt asked for it, despite 31.7% of real prompts requesting audio. |
