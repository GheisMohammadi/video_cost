#!/usr/bin/env bash
# Runs ON THE POD: arm the independent guard first, then install deps and run the round 3
# batch session. Files expected in /root: pod_guard.sh, batch_session.py, round3_groups.json.
# Logs: /root/out/guard.log, /root/out/session.log, results in /root/out/session/
# Weights go to /root/hf (local container disk).
mkdir -p /root/out/session /root/hf
export WAN_CACHE=/root/hf
rm -f /root/out/SESSION_DONE
HARD_S=${HARD_S:-6000} GRACE_S=${GRACE_S:-900} PROC_PATTERN='[b]atch_session' nohup bash /root/pod_guard.sh > /root/out/guard.out 2>&1 &
sleep 2
...
python3 /root/batch_session.py /root/round3_groups.json --out /root/out/session --gpu-usd-h "${GPU_USD_H:-1.6}" --disk-usd-h "${DISK_USD_H:-0.028}" > /root/out/session.log 2>&1
code=$?
